...
const renderSidebar = async () => {
    ...
    if (appSidebar) {
        if (user.isUsuario || user.isInvitado || window.location.pathname === "/tienda") {
            ...
            const isActive = (path: string): string => currentPath === path ? 'class="active"' : '';
            appSidebar.innerHTML = `
            <h3 class="sidebar-title">Administracíon</h3>
            <h4 class="sidebar-subtitle">Panel de control</h4>
            <ul class="sidebar-menu">
                <li ${isActive('/adminPanel')}><a href="/adminPanel">Dashboard</a></li>
                <li ${isActive('/manageCategories')}><a href="/manageCategories">Categorías</a></li>
                <li ${isActive('/manageProducts')}><a href="/manageProducts">Productos</a></li>
                <li ${isActive('/manageOrders')}><a href="/manageOrders">Pedidos</a></li>
